<template>
  <div :class="`text-white container mx-auto`">
    ...
    <div :class="`relative p-4 rounded-tr-3xl rounded-bl-3xl sm:m-auto sm:w-3/4 border-2 border-[color:var(--naranja)] bg-[color:var(--gris)] overflow-hidden`" v-html="user.config?.config?.terms_conditions?? 'En construcción.'" />
    <br>
  </div>
</template>
